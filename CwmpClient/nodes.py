    ...
    def __call__(self, *args: Any, **kwds: Any) -> Any:
        if len(args) == 1:
            ...
            if self.type == int:
                ...
                new_value = new_value if type(
                    new_value) == bool else new_value == "1"
    ...
    def addItem(self, key: str, value: any, writable: bool = True) -> None:
        ...
        if parameter in self.config[path]:
            ...
            node[parameter] = Parameter(
                value_type, value, writable=writable, setter=setter)
        elif type(value) == type:
            node[parameter] = Parameter(
                value, writable=writable, setter=setter)
        else:
            node[parameter] = Parameter(
                type(value), value, writable=writable, setter=setter)
...
class ParameterList:
    ...
    def _get_parameters(self, currentNode: BaseNode, path: typing.List[str], prefix: str = "") -> None:
        if isinstance(currentNode, Parameter):
            self.parameters.update({prefix: currentNode})
        elif len(path) == 0:
            self.parameters.update({(prefix + '.' if len(prefix) > 0 else '') +
                                   name: child for name, child in currentNode.childs.items()})
        else:
            node_name = path.pop(0)
            if len(node_name) == 0:
                self.parameters.update(
                    {prefix + '.' + name: child for name, child in currentNode.childs.items()})
            elif node_name == '*':
                for node_name in currentNode.childs:
                    self._get_parameters(
                        currentNode[node_name], path, prefix + '.' + node_name if len(prefix) > 0 else node_name)
            else:
                self._get_parameters(
                    currentNode[node_name], path, prefix + '.' + node_name if len(prefix) > 0 else node_name)
...
class ParameterListValue(ParameterList):
    def todom(self, document):
        ...
        pl.setAttributeNS(SOAPNS, 'soap:arrayType',
                          "cwmp:ParameterValueStruct[%d]" % len(self.parameters))
        for key, value in self.parameters.items():
            ...
            if value.type == bool:
                value_node.appendChild(
                    document.createTextNode("1" if value() else "0"))
                ...
                if value.type == int:
                    ...
                    value_node.setAttributeNS(
                        XSINS, 'xsi:type', 'xsd:' + value.type)
                else:
                    log.error(
                        "Unknown type conversion from %s to XML", value.type)
                    value_node.setAttributeNS(XSINS, 'xsi:type', 'xsd:string')
...
class ParameterListName(ParameterList):
    def todom(self, document):
        pl = document.createElement('ParameterList')
        pl.setAttributeNS(SOAPNS, 'soap:arrayType',
                          "cwmp:ParameterInfoStruct[%d]" % len(self.parameters))
        for key, value in self.parameters.items():
            pis = document.createElement('ParameterInfoStruct')
            name = document.createElement('Name')
            name.appendChild(document.createTextNode(
                key if isinstance(value, Parameter) else key + "."))
            pis.appendChild(name)
            writable = document.createElement('Writable')
            writable.appendChild(document.createTextNode(
                "1" if value.writable else "0"))

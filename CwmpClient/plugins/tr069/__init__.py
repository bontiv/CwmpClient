...
async def loader(rootnode):
    ...
    config.addItem('Device.ManagementServer.NATDetected', False, writable=False)
    config.addItem('Device.ManagementServer.STUNServerAddress', "stun.l.google.com")
    config.addItem('Device.ManagementServer.STUNServerPort', 19302)
    config.addItem('Device.ManagementServer.UDPConnectionRequestAddress', "", writable=False)
    config.write()
...
async def start(app):
    ...
    app.root['Device']['ManagementServer']['ConnectionRequestURL']('http://%s:%d/%s' % (
        ...
        7547,
        "".join([random.choice("acbdefghijhlmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXIZ0123456789") for i in range(32)])
        ))
